...
export default function Page(){
 const [data,setData]=useState<any>(),router=useRouter();
 ...
 return <PortalGuard role="staff"><div className="min-h-screen bg-mist">
  <header className="bg-navy-deep text-white"><div className="max-w-[1100px] mx-auto px-4 sm:px-7 h-20 flex items-center justify-between"><div><p className="text-on-blue text-xs uppercase tracking-widest font-bold">UpTech</p><h1 className="text-xl">Staff Portal</h1></div><button onClick={logout} className="rounded-full bg-white/10 px-4 py-2 font-bold">Sign out</button></div></header>
  <main className="max-w-[1100px] mx-auto px-4 sm:px-7 py-8 sm:py-12">{!p?<p>Loading your dashboard…</p>:<>
   <section className="bg-white rounded-card border border-line shadow-card p-6 sm:p-8"><div className="flex gap-4 items-center"><div className="w-14 h-14 rounded-card-sm bg-brand/10 text-brand grid place-items-center"><UserRound/></div><div><p className="text-ink-soft">Welcome back</p><h2 className="text-2xl">{p.firstName} {p.lastName}</h2></div></div><dl className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-8"><Info k="Employee ID" v={p.employeeId}/><Info k="Position" v={p.jobTitle}/><Info k="Department" v={p.department||"—"}/><Info k="Start date" v={p.startDate||"—"}/></dl></section>
