...
export async function GET(request: NextRequest) {
  const user = await requireRole(["staff", "hr", "super_admin"]);
  ...
  if (user.role !== "super_admin" && requested !== null) return NextResponse.json({message:"You can only view your own certificate."}, {status:403, headers});
  const uid = user.role === "super_admin" ? requested : user.uid;
  if (!uid || uid.includes("/") || uid.length > 128) return NextResponse.json({message:"Select a member."}, {status:400, headers});
  try {
    const [profile, account] = await Promise.all([
      adminFirestore.collection("staffProfiles").doc(uid).get(),
      adminFirestore.collection("users").doc(uid).get(),
    ]);
    if (!profile.exists) return NextResponse.json({message:"Member record not found."}, {status:404, headers});
